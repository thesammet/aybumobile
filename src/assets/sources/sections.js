export const sections = [
    {
        "faculty": "Mühendislik ve Doğa Bilimleri Fakültesi",
        "departments": [
            { "name": "Bilgisayar Mühendisliği" },
            { "name": "Elektrik-Elektronik Mühendisliği" },
            { "name": "Endüstri Mühendisliği" },
            { "name": "Enerji Sistemleri Mühendisliği" },
            { "name": "İnşaat Mühendisliği" },
            { "name": "Makine Mühendisliği" },
            { "name": "Metalurji ve Malzeme Mühendisliği" },
            { "name": "Matematik Mühendisliği" },
            { "name": "Yazılım Mühendisliği" },
        ]
    },
    {
        "faculty": "Diş Hekimliği Fakültesi",
        "departments": [
            { "name": "Diş Hekimliği" },
        ]
    },
    {
        "faculty": "Havacılık ve Uzay Bilimleri Fakültesi",
        "departments": [
            { "name": "Havacılık ve Uzay Mühendisliği" },
            { "name": "Havacılık Yönetimi" },
        ]
    },
    {
        "faculty": "Hukuk Fakültesi",
        "departments": [
            { "name": "Hukuk" },
        ]
    },
    {
        "faculty": "İnsan ve Toplum Bilimleri Fakültesi",
        "departments": [
            { "name": "Bilgi ve Belge Yönetimi Bölümü" },
            { "name": "Doğu Dilleri ve Edebiyatları Bölümü" },
            { "name": "Felsefe Bölümü" },
            { "name": "Psikoloji Bölümü" },
            { "name": "Sosyoloji Bölümü" },
            { "name": "Tarih Bölümü" },
            { "name": "Sanat Tarihi Bölümü" },
            { "name": "Türk Dili ve Edebiyatı Bölümü" },
            { "name": "Bilim Tarihi Bölümü" },
            { "name": "Mütercim ve Tercümanlık Bölümü" },
        ]
    },
    {
        "faculty": "İslami İlimler Fakültesi",
        "departments": [
            { "name": "Temel İslam Bilimleri Bölümü" },
            { "name": "Felsefe ve Din Bilimleri Bölümü" },
            { "name": "İslam Tarihi ve Sanatları Bölümü" },
        ]
    },
    {
        "faculty": "İşletme Fakültesi",
        "departments": [
            { "name": "Finans ve Bankacılık" },
            { "name": "İşletme" },
            { "name": "Uluslararası Ticaret ve İşletmecilik" },
            { "name": "Yönetim Bilişim Sistemleri" },
        ]
    },
    {
        "faculty": "Mimarlık ve Güzel Sanatlar Fakültesi",
        "departments": [
            { "name": "Mimarlık" },
            { "name": "Görsel İletişim Tasarımı" },
            { "name": "Endüstriyel Tasarım" },
        ]
    },
    {
        "faculty": "Sağlık Bilimleri Fakültesi",
        "departments": [
            { "name": "Beslenme ve Diyetetik" },
            { "name": "Çocuk Gelişimi" },
            { "name": "Dil ve Konuşma Terapisi" },
            { "name": "Fizyoterapi ve Rehabilitasyon" },
            { "name": "Hemşirelik" },
            { "name": "Odyoloji" },
            { "name": "Sağlık Yönetimi" },
            { "name": "Sosyal Hizmet" },
        ]
    },
    {
        "faculty": "Siyasal Bilgiler Fakültesi",
        "departments": [
            { "name": "İktisat Bölümü" },
            { "name": "Maliye Bölümü" },
            { "name": "Siyaset Bilimi ve Kamu Yönetimi Bölümü" },
            { "name": "Uluslararası İlişkiler Bölümü" },
        ]
    },
    {
        "faculty": "Spor Bilimleri Fakültesi",
        "departments": [
            { "name": "Antrenörlük Eğitimi Bölümü" },
            { "name": "Egzersiz ve Spor Bilimleri Bölümü" },
            { "name": "Spor Yöneticiliği Bölümü" },
        ]
    },
    {
        "faculty": "Şereflikoçhisar Uygulamalı Bilimler Fakültesi",
        "departments": [
            { "name": "Finans ve Bankacılık" },
            { "name": "Uluslararası Ticaret ve Lojistik" },
        ]
    },
    {
        "faculty": "Tıp Fakültesi",
        "departments": [
            { "name": "Cerrahi Tıp Bilimleri" },
            { "name": "Dahili Tıp Bilimleri" },
            { "name": "Temel Tıp Bilimleri" },
        ]
    },
    {
        "faculty": "Türk Musikisi Devlet Konservatuvarı",
        "departments": [
            { "name": "Türk Müziği Bölümü" },
        ]
    },
]